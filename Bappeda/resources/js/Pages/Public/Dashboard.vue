...
import { computed } from 'vue'; // Hapus ref, pakai computed
...
const props = defineProps({
    stats: Object,       // { total_dataset, data_valid, total_visual, total_org, total_upload }
    chartThemes: Object,
    chartBidang: Object, // { labels: [], data: [] }
    datasets: Object,    // { popular: [], latest: [] }
    topics: Array        // [{ name: 'Sosial' }, ...]
});

// 2. KONFIGURASI STATS CARDS (Gabungkan Icon Static dengan Data Dynamic)
const statsCards = computed(() => [
    { 
        label: 'TOTAL DATASET', 
        value: props.stats.total_dataset, // Data dari DB
    ...
    { 
        label: 'DATA VALID', 
        value: props.stats.data_valid, 
        icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z', 
        color: 'green', 
        progress: (props.stats.data_valid / (props.stats.total_upload || 1)) * 100 
    },
    { 
        label: 'TOTAL VISUALISASI', 
        value: props.stats.total_visual, 
...
const barChartData = computed(() => ({
    labels: props.chartThemes.labels, // Label dari DB (Nama Tema)
    datasets: [{
        label: 'Jumlah Indikator',
        backgroundColor: '#000B58',
        borderRadius: 6,
        data: props.chartThemes.data // Data dari DB (Count)
    }]
}));
...
// 4. CHART DOUGHNUT (DYNAMIC)
const percentValid = computed(() => {
    return props.stats.total_upload > 0 
        ? Math.round((props.stats.data_valid / props.stats.total_upload) * 100) 
        : 0;
});

const doughnutColors = ['#00139E', '#2563EB', '#3B82F6', '#60A5FA', '#93C5FD', '#E2E8F0'];

const doughnutData = computed(() => ({
    labels: props.chartBidang.labels,
    datasets: [{
        data: props.chartBidang.data,
        backgroundColor: doughnutColors, // Gunakan variasi biru
        borderWidth: 0, 
        cutout: '75%'
    }]
}));

// 5. TOPIK ICON (Mapping Manual Icon ke Nama Tema dari DB)
// Karena icon SVG panjang, kita simpan list iconnya saja, lalu pasangkan dengan data DB
...
const topicList = computed(() => {
    return props.topics.map((topic, index) => ({
        name: topic.name,
        // Ambil icon berdasarkan urutan, jika habis ulang dari awal
        icon: availableIcons[index % availableIcons.length] 
    }));
});

const totalBidangData = computed(() => {
    return props.chartBidang.data.reduce((a, b) => a + b, 0);
});
    ...
    <div class="space-y-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
       ...
       <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white p-8 rounded-[1.5rem] shadow-sm border border-gray-100 flex flex-col">
            <div class="flex justify-between items-center mb-8">
                <h3 class="text-lg font-bold text-gray-800 tracking-tight">Distribusi Bidang</h3>
            ...
            <div class="flex-1 relative flex items-center justify-center px-4 min-h-[200px]">
                ...
                <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                    <span class="text-4xl font-black text-[#00139E]">{{ totalBidangData }}</span>
                    <span class="text-[10px] font-bold text-gray-400 uppercase">Total Data</span>
                </div>
            </div>

            <div class="mt-8 space-y-3 pt-6 border-t border-gray-50 max-h-[150px] overflow-y-auto custom-scrollbar">
                <div v-for="(label, idx) in chartBidang.labels" :key="idx" class="flex items-center justify-between text-xs">
                    <div class="flex items-center gap-2">
                        <div class="w-3 h-3 rounded-full" :style="{ backgroundColor: doughnutColors[idx % doughnutColors.length] }"></div>
                        <span class="font-bold text-gray-600 truncate max-w-[120px]">{{ label }}</span>
                    </div>
                    <span class="font-black text-gray-800">{{ chartBidang.data[idx] }}</span>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="py-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
                <a href="#" v-for="(topic, index) in topicList" :key="index" class="group bg-white rounded-2xl p-6 border border-gray-100 shadow-sm hover:shadow-md hover:border-blue-100 transition flex flex-col items-center justify-center text-center">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="bg-white rounded-[1.5rem] p-8 shadow-sm border border-gray-100">
                ...
                <div class="space-y-6">
                    <div v-for="(item, index) in props.datasets.popular" :key="index" class="flex items-start gap-4 group">
                        ...
                        <div>
                             <Link :href="`/dataset/${item.id}`" class="text-sm font-bold text-gray-800 group-hover:text-blue-600 transition line-clamp-2 leading-snug mb-2">{{ item.title }}</Link>
            ...
            <div class="bg-white rounded-[1.5rem] p-8 shadow-sm border border-gray-100">
                ...
                <div class="space-y-6">
                    <div v-for="(item, index) in props.datasets.latest" :key="index" class="flex items-start gap-4 group">
