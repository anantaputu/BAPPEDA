...
const confirmDelete = (item) => {
    selectedId.value = item.id_katakunci
    selectedTitle.value = item.nama_katakunci
    showModal.value = true
}

const deleteData = () => {
    if (selectedId.value) {
        router.delete(`/admin/katakunci/${selectedId.value}`, {
            onSuccess: () => {
...
<template>
    <Head title="Manajemen Kata Kunci" />

    <div class="min-h-full">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8">
            <div>
                <h1 class="text-2xl font-black text-primary uppercase tracking-tight">
                    Manajemen <span class="text-secondary">Kata Kunci</span>
                </h1>
                <p class="text-sm text-textsecondary font-medium mt-1">
                    Total Indeks Tag: <span class="text-primary font-black">{{ katakunci.length }}</span> Label
                </p>
            </div>

            <Link
                href="/admin/katakunci/create"
        ...
        <div class="overflow-hidden rounded-xl border border-gray-400 bg-white shadow-sm">
            <table class="w-full border-collapse">
                ...
                <tbody class="text-sm font-bold text-primary">
                        ...
                        <td class="p-8">
                            <div class="flex items-center gap-4">
                                ...
                                <div class="flex flex-col">
                                    <span class="font-black uppercase text-xs tracking-tight text-primary">{{ k.nama_katakunci }}</span>
                                    <span class="text-[9px] text-textsecondary opacity-50 uppercase font-bold tracking-tighter">ID: #{{ k.id_katakunci }}</span>
                                </div>
                            </div>
                        </td>

                        <td v-if="cantManage" class="p-8 text-right space-x-3">
                            <Link :href="`/admin/katakunci/${k.id_katakunci}/edit`" 
                    ...
                    <tr v-if="katakunci.length === 0">
                        <td colspan="2" class="p-24 text-center">
                             <div class="flex flex-col items-center opacity-30">
                                <svg class="w-12 h-12 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4" />
                                </svg>
                                <p class="text-[10px] font-black text-textsecondary uppercase tracking-[0.3em]">Belum ada indeks kata kunci</p>
                             </div>
                        </td>
                    </tr>
                </tbody>
            ...
            <div v-if="showModal" class="fixed inset-0 z-[100] flex items-center justify-center p-6">
                ...
                <div class="relative bg-white w-full max-w-sm rounded-xl p-8 shadow-2xl border border-gray-200">
                    ...
                    <div class="text-center mb-8">
                        <h3 class="text-xl font-black text-primary uppercase tracking-tight mb-2">Hapus Kata Kunci?</h3>
                        <p class="text-textsecondary text-xs font-bold leading-relaxed px-4 opacity-70">
                            Menghapus label <span class="text-primary font-black">"{{ selectedTitle }}"</span> akan melepaskan keterkaitan tag pada semua data yang menggunakannya.
                        </p>
                    </div>
