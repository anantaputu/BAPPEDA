...
const confirmDelete = (item) => {
    selectedId.value = item.id_data
    selectedTitle.value = item.nama_indikator
    ...
    <div class="bg-white rounded-[2.5rem] p-10 shadow-2xl shadow-gray-100 border border-gray-400 min-h-[70vh]">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-12">
            <div>
                <h1 class="text-4xl font-black text-gray-900 tracking-tight">
                    Data <span class="text-[#00139E]">Indikator</span>
                </h1>
                <p class="text-xs text-gray-400 font-bold uppercase tracking-[0.2em] mt-2 flex items-center gap-2">
                    ...
                    Total Terdata: {{ data.total }} Parameter
                </p>
            </div>
            ...
            <table class="w-full border-collapse">
                <thead>
                    <tr class="bg-gray-50/50 text-left text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] border-b border-gray-400">
                        <th class="p-8">Informasi Indikator</th>
                        <th>Sektoral / Tema</th>
                        <th>Urusan</th>
                        <th>Status</th>
                ...
                <tbody class="text-sm font-bold text-gray-800">
                    <tr v-for="item in data.data" :key="item.id_data" 
                        class="border-b border-gray-400 last:border-0 hover:bg-blue-50/20 transition-all group">
                        
                        <td class="p-8">
                            <div class="max-w-xs">
                                <p class="text-gray-900 font-black uppercase text-[11px] leading-tight mb-1">{{ item.nama_indikator }}</p>
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-wider">ID: #{{ item.id_data }}</p>
                            </div>
                        </td>

                        <td>
                            <span class="bg-gray-100 text-gray-500 px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest border border-gray-200">
                                {{ item.tema?.nama_tema || '-' }}
                            </span>
                        </td>

                        <td>
                            <p class="text-[10px] text-gray-400 uppercase leading-snug max-w-[180px] font-medium">
                        ...
                        <td class="p-8 text-right space-x-3">
                            <Link :href="`/admin/data/${item.id_data}/edit`" 
                    ...
                    <tr v-if="data.data.length === 0">
                        <td colspan="5" class="p-20 text-center text-gray-300 uppercase italic tracking-widest text-xs">
                            Belum ada data indikator yang tersedia
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div class="mt-12 flex flex-wrap justify-center items-center gap-3">
            <template v-for="(link, index) in data.links" :key="index">
                <div v-if="link.url === null" v-html="link.label" class="px-4 py-2 text-gray-300 text-[10px] font-black uppercase tracking-widest"></div>
                <Link v-else :href="link.url" v-html="link.label" 
                    class="px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-[0.15em] transition-all active:scale-90 shadow-sm"
                    :class="link.active 
                        ? 'bg-[#00139E] text-white shadow-blue-500/20' 
                        : 'bg-white text-gray-400 border border-gray-200 hover:bg-gray-50 hover:text-[#00139E]'"
                />
            </template>
        </div>
        ...
        >
            <div v-if="showModal" class="fixed inset-0 z-[100] flex items-center justify-center p-6">
                ...
                <div class="relative bg-white w-full max-w-md rounded-[2.5rem] p-10 shadow-2xl text-center transform transition-all border border-gray-100">
                    ...
                    <h3 class="text-2xl font-black text-[#000B58] mb-2">Hapus Indikator?</h3>
                    <p class="text-[#A2B5CB] text-sm leading-relaxed mb-10 px-4">
                        Anda akan menghapus data <span class="text-gray-900 font-bold">"{{ selectedTitle }}"</span> secara permanen. Pastikan data ini tidak lagi dibutuhkan oleh sistem.
                    </p>
