...
// Menggunakan useForm untuk prefill data dan sinkronisasi loading/error state
const form = useForm({
    nama_tema: props.tema.nama_tema ?? ''
})

const submit = () => {
    form.put(`/admin/tema/${props.tema.id_tema}`)
}
</script>

<template>
    <Head title="Edit Tema Sektoral" />
        ...
        <div class="bg-white rounded-[2.5rem] p-12 shadow-2xl shadow-gray-100 border border-gray-400">
            <div class="mb-10">
                <h1 class="text-4xl font-black text-gray-900 tracking-tight">
                    Edit <span class="text-[#00139E]">Tema Sektoral</span>
                </h1>
                <p class="text-gray-400 font-medium mt-2">Perbarui klasifikasi tema untuk data indikator pembangunan. ID: #{{ props.tema.id_tema }}</p>
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-8">
                <div class="space-y-6">
                    <div class="space-y-2">
                        ...
                        <p v-if="form.errors.nama_tema" class="text-red-500 text-xs ml-4 font-bold">{{ form.errors.nama_tema }}</p>
                    </div>
                    ...
                    <div class="bg-amber-50 border border-amber-100 rounded-2xl p-6 flex items-start gap-4">
                        ...
                        <p class="text-[11px] text-amber-700 leading-relaxed font-medium">
                            Perubahan pada nama tema akan berdampak langsung pada seluruh filter dan label di halaman dashboard publik serta seluruh data indikator yang terelasi.
                        </p>
                    </div>
                </div>
