...
const form = useForm({
    nama_bidang: props.bidang.nama_bidang ?? ''
})

const submit = () => {
    form.put(`/admin/bidang/${props.bidang.id_bidang}`)
}
</script>

<template>
    <Head title="Edit Bidang Kerja" />

    <div class="max-w-3xl mx-auto">
        <Link href="/admin/bidang" class="flex items-center gap-2 text-[#A2B5CB] hover:text-[#00139E] transition-colors mb-6 group">
            <svg class="w-5 h-5 transform group-hover:-translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
            </svg>
            <span class="font-bold uppercase tracking-widest text-[10px]">Kembali ke Daftar Bidang</span>
        </Link>

        <div class="bg-white rounded-[2.5rem] p-12 shadow-2xl shadow-gray-100 border border-gray-400">
            <div class="mb-10">
                <h1 class="text-4xl font-black text-gray-900 tracking-tight">
                    Edit <span class="text-[#00139E]">Bidang Kerja</span>
                </h1>
                <p class="text-gray-400 font-medium mt-2">Perbarui nomenklatur bidang atau instansi pengelola. ID: #{{ props.bidang.id_bidang }}</p>
            </div>
                ...
                <div class="space-y-6">
                    <div class="space-y-2">
                        <label class="text-[10px] font-black uppercase tracking-widest text-[#00139E] ml-4">
                            Nama Bidang / Instansi <span class="text-red-500">*</span>
                        </label>
                        <div class="relative">
                            <input 
                                ...
                                :class="{ 'border-red-500': form.errors.nama_bidang }"
                                required 
                            />
                        </div>
                        <p v-if="form.errors.nama_bidang" class="text-red-500 text-xs ml-4 font-bold">{{ form.errors.nama_bidang }}</p>
                    </div>
                    ...
                    <div class="bg-amber-50 border border-amber-100 rounded-2xl p-6 flex items-start gap-4">
                        ...
                        <p class="text-[11px] text-amber-700 leading-relaxed font-medium">
                            Perubahan pada nama bidang akan otomatis memperbarui label kategori pada setiap Inputer dan Indikator yang terasosiasi. Pastikan data tetap sinkron dengan struktur organisasi terbaru.
                        </p>
                    </div>
                </div>

                <div class="pt-6 flex items-center justify-end gap-4 border-t border-gray-50">
                    <Link href="/admin/bidang" class="px-8 py-4 text-gray-400 font-bold hover:text-gray-600 transition-colors uppercase tracking-widest text-[10px]">
                        Batal
                    </Link>
                    <button 
                        type="submit" 
